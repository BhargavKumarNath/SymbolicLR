...
with st.sidebar:
    ...
    with st.container():
        ...
        max_gen = 20 if cfg.is_cloud else 50
        max_pop = 100 if cfg.is_cloud else 200
        gen_count = st.slider("Generations", 1, max_gen, min(5, max_gen))
        pop_size  = st.slider("Population Size", 10, max_pop, min(30, max_pop))
        epochs    = st.slider("Epochs per Eval", 1, 5, 1)
        workers   = st.slider("Parallel Workers", 1, 8, 1 if cfg.is_cloud else 4)
    ...
    if is_running:
        @st.fragment(run_every=cfg.fragment_interval_s)
        def _live_progress():
            sync_state()
            ...
            <div class="sidebar-stat">
                ...
                <div style="font-family:var(--font-body);font-size:12px;
                            color:var(--text-secondary);margin-top:4px;">{phase}</div>
            </div>
            """, unsafe_allow_html=True)
            if total > 0:
